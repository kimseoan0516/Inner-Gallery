...
[강조 표현 규칙]:
본문 전체에서 감상자의 마음에 가장 깊이 닿을 핵심 문장 2~3개를 ==문장== 형식으로 감싸라.
예: ==이 그림은 서두르지 않아도 된다고, 지금 이 자리에 그냥 있어도 된다고 말하는 것 같습니다.==
- 반드시 완전한 한 문장(15자 이상) 단위로 감싸야 한다. 단어나 짧은 구절은 안 된다.
- 문단의 마지막 문장처럼 감상자에게 가장 인상 깊게 남을 문장을 고른다.
- 본문 전체에 총 2~3개만 사용한다. 남용 금지.
- 작품 제목에는 사용하지 않는다 (작품 제목은 「」 사용).
