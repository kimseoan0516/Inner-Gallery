...
schema_target = '''            CREATE TABLE IF NOT EXISTS artist_quotes (
                id     INTEGER PRIMARY KEY AUTOINCREMENT,
                quote  TEXT NOT NULL,
                artist TEXT NOT NULL
            )'''
schema_replace = '''            CREATE TABLE IF NOT EXISTS artist_quotes (
                id        INTEGER PRIMARY KEY AUTOINCREMENT,
                quote     TEXT NOT NULL,
...
content = content.replace(schema_target, schema_replace)

# Replace pg migrations
migration_target = '''        if _USE_PG:
            c.execute("ALTER TABLE journal_entries ADD COLUMN IF NOT EXISTS era_data TEXT DEFAULT ''")'''
migration_replace = '''        if _USE_PG:
            ...
            c.execute("ALTER TABLE journal_entries ADD COLUMN IF NOT EXISTS era_data TEXT DEFAULT ''")'''
content = content.replace(migration_target, migration_replace)

# Replace sqlite migrations
sqlite_migration_target = '''        else:
            for col, default in [("era_data", "''"), ("ticket_exhibition", "''"), ("question_answers", "'{}'")]:'''
sqlite_migration_replace = '''        else:
            for col, default in [("quote_en", "''"), ("artist_en", "''")]:
                try: c.execute(f"ALTER TABLE artist_quotes ADD COLUMN {col} TEXT DEFAULT {default}")
                except: pass
            for col, default in [("era_data", "''"), ("ticket_exhibition", "''"), ("question_answers", "'{}'")]:'''
content = content.replace(sqlite_migration_target, sqlite_migration_replace)

# Replace seed check logic (force re-seed on restart to apply updates)
seed_target = '''        # 명언 초기 데이터 (테이블이 비어 있을 때만 삽입)
        count = c.execute("SELECT COUNT(*) FROM artist_quotes").fetchone()
        if (count[0] if isinstance(count, tuple) else list(count.values())[0]) == 0:
            _seed_quotes(c)'''
seed_replace = '''        # 명언 초기 데이터 (기존 데이터 삭제 후 갱신)
        c.execute("DELETE FROM artist_quotes")
        _seed_quotes(c)'''
content = content.replace(seed_target, seed_replace)

# Replace _QUOTES_DATA array
new_quotes_str = '_QUOTES_DATA = [\n'
for q, q_en, a, a_en in QUOTES:
    # escape quotes
    q = q.replace("'", "\\'")
    q_en = q_en.replace("'", "\\'")
    a = a.replace("'", "\\'")
    a_en = a_en.replace("'", "\\'")
    new_quotes_str += f'    ("{q}", "{q_en}", "{a}", "{a_en}"),\n'
new_quotes_str += ']'
content = re.sub(r'_QUOTES_DATA = \[.*?\]', new_quotes_str, content, flags=re.DOTALL)

# Update _seed_quotes
seed_q_target = '''def _seed_quotes(c):
    for quote, artist in _QUOTES_DATA:
        c.execute("INSERT INTO artist_quotes (quote, artist) VALUES (?, ?)", (quote, artist))'''
seed_q_replace = '''def _seed_quotes(c):
...
content = content.replace(seed_q_target, seed_q_replace)

# Update get_random_quote
get_target = '''def get_random_quote() -> dict:
    with conn() as c:
...
get_replace = '''def get_random_quote() -> dict:
    with conn() as c:
