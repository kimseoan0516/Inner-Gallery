...
사용자가 작품을 촬영하거나 업로드하면, 시스템은 작품을 식별하고 시각적 특징을 분석합니다. 그리고 감상 질문·AI 해설·마음색·마음 스케치·회고문으로 이어지는 감정 여정을 제공하고, 그 순간의 마음은 전시 티켓 형태로 개인 아카이브에 남습니다.

이 프로젝트에서 가장 중요하게 생각한 것은 두 가지입니다.

### 1. 감성적인 AI가 근거 없는 해석을 하지 않을 것

작품 식별 결과, 컴퓨터 비전 분석 데이터, 직접 구축한 작품 맥락 데이터베이스를 기반으로 해설을 생성하며, 보이지 않는 표정이나 의도를 상상하지 않도록 설계했습니다.

### 2. 감상 이후의 감정을 사라지지 않게 기록할 것

감상 전후 감정, 마음색, 마음 스케치, AI 회고문까지 하나의 기록으로 저장해, 사용자가 자신의 감정 변화를 시간의 흐름 속에서 다시 돌아볼 수 있도록 설계했습니다.
...
*어쩌면 우리는 그림을 이해하려고 미술관에 가는 것이 아니라, </br>
...
> **평가 조건**  
> 채점 기준은 작가명 Top-1 일치(substring match)이며, 평가 스크립트(`proper_eval.py`, seed=42)는 저장소에 포함되어 재현 가능합니다. 오답 18건 중 2건은 `Albrecht Dürer` 폴더명 인코딩 불일치로 인한 채점 오류이며, 이를 제외하면 실질 정확도는 **84/98 ≈ 85.7%**입니다.
