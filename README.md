...
사용자 흐름은 작품 이미지 업로드에서 시작해 전시 티켓 저장으로 끝납니다.

```
작품 이미지 업로드 / 촬영
        ↓
[카메라] 화질 검사 → 프레임 감지 & 원근 보정
[업로드] 프레임 감지 & 원근 보정
        ↓
하이브리드 작품 식별
CLIP+FAISS · Gemini Vision · Web Detection · OCR 교차 검증
        ↓
색채 · 구도 · 인물 컴퓨터 비전 분석 + 6D 감정 스코어링
        ↓
Grounded AI 도슨트 해설 + 감상 질문      내 감정 상태와 연결된 해설
        ↓
마음색 선택                              오늘의 감정을 색 하나로 기록
        ↓
마음 스케치                              선·색·문장으로 마음의 흔적을 남김
        ↓
AI 스케치 회고문                         Gemini Vision이 스케치를 읽고 쓰는 회고
        ↓
전시 티켓으로 저장                        No. IG-YYMMDD-XXXX
```
